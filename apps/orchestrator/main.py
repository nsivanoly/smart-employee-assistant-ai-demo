...
from orchestrator.agents.routes import AgentsRouterDeps, build_agents_router
from orchestrator.trace.recorder import HttpTraceRecorder
from orchestrator.trace.routes import TraceRouterDeps, build_trace_router
...
def create_app(config: OrchestratorConfig | None = None) -> FastAPI:
    ...
    # Instrument httpx (outbound HTTP spans + traceparent for the A2A/peer
    # chain). Gated by ENABLE_API_TRACES (default off) — otherwise every
    # outbound call (proxy to hr_server/it_server: /api/me/*, /api/reports/*)
    # is spanned, which is noisy. Enable to get the orchestrator->hr->it chain.
    if os.getenv("ENABLE_API_TRACES", "false").strip().lower() in ("1", "true", "yes"):
        try:
            from opentelemetry.instrumentation.httpx import HTTPXClientInstrumentor
            HTTPXClientInstrumentor().instrument()
            logger.info("httpx_instrumentation_enabled")
        except Exception as exc:  # noqa: BLE001
            logger.warning("httpx_instrumentation_skipped reason=%r", exc)
    else:
        logger.info("httpx_instrumentation_disabled (set ENABLE_API_TRACES=1 to enable)")
    ...
    # Under-the-hood HTTP trace recorder — installed as httpx event hooks on
    # every outbound client so the SPA can show, per chat request, the real
    # orchestrator → agent request/response/headers. Sensitive headers are
    # masked inside the recorder; raw token bytes never reach the browser.
    http_trace_recorder = HttpTraceRecorder()
    _trace_hooks = http_trace_recorder.event_hooks()
    a2a_clients: dict[str, A2AClient] = {
        "hr_agent": A2AClient(
            A2AClientConfig(base_url=cfg.hr_agent_url),
            http=httpx.AsyncClient(event_hooks=_trace_hooks),
        ),
        "it_agent": A2AClient(
            A2AClientConfig(base_url=cfg.it_agent_url),
            http=httpx.AsyncClient(event_hooks=_trace_hooks),
        ),
    }
    ...
    async def lifespan(_app: FastAPI) -> AsyncIterator[None]:
        ...
        reports_http_client = httpx.AsyncClient(timeout=10.0, event_hooks=_trace_hooks)
        ...
        actor_provider = ActorTokenProvider(
            ...
            is_client=is_client,
            # Pattern-C can effectively be scope-capped by the actor token.
            # Keep the actor token envelope aligned with the user-authorize
            # business scopes so token-A can carry HR/IT scopes for reports.
            scope=(
                "openid profile email internal_login "
        ...
        for agent_id, client in a2a_clients.items():
            # The A2A clients were built with an injected httpx client (so the
            # trace hooks are installed); aclose() won't close an injected
            # client, so drain its transport explicitly.
            await client.aclose()
    ...
    # Server-side API request tracing (incoming HTTP). Off by default — it spans
    # every request including noisy /healthz and /events SSE long-polls. Enable
    # with ENABLE_API_TRACES=1 to get cross-service span nesting (traceparent
    # extraction). /healthz and /events are always excluded even when enabled.
    if os.getenv("ENABLE_API_TRACES", "false").strip().lower() in ("1", "true", "yes"):
        try:
            from opentelemetry.instrumentation.fastapi import FastAPIInstrumentor
            FastAPIInstrumentor.instrument_app(app, excluded_urls="healthz,events")
            logger.info("fastapi_instrumentation_enabled")
        except Exception as exc:  # noqa: BLE001
            logger.warning("fastapi_instrumentation_skipped reason=%r", exc)
    else:
        logger.info("fastapi_instrumentation_disabled (set ENABLE_API_TRACES=1 to enable)")
    ...
    # ── Agents router (fleet status + token-termination demo) ─────────────────
    # Reuses the same late-binding httpx proxy (for /healthz polls) and the
    # Sprint 3 fan-out client (for the Terminate action) already wired above.
    app.include_router(
        build_agents_router(
            AgentsRouterDeps(
                session_store=session_store,
                agent_registry=agent_registry,
                session_cookie_name=cfg.session_cookie_name,
                http_client=proxy_reports_http,
                events_client=events_client,
                agent_urls={
                    "hr_agent": cfg.hr_agent_url,
                    "it_agent": cfg.it_agent_url,
                },
                orchestrator_label="Orchestrator (BFF)",
                orchestrator_client_id=cfg.mcp_client_id,
            )
        )
    )

    # ── Trace router (under-the-hood HTTP capture) ────────────────────────────
    # Surfaces the outbound calls captured by `http_trace_recorder` so the SPA
    # trace panel can show real request/response/headers per X-Request-ID.
    app.include_router(
        build_trace_router(
            TraceRouterDeps(
                session_store=session_store,
                recorder=http_trace_recorder,
                session_cookie_name=cfg.session_cookie_name,
            )
        )
    )
