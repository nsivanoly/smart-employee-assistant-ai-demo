    ...
    for tool_index, tool_call in enumerate(tool_calls):
        with _tracer.start_as_current_span(
            ...
            if isinstance(second, ResultPayload):
                ...
                # Flush so the Agents-panel token view survives a reload.
                ...
                event_message = second.error_id
                if second.reason:
                    event_message = f"{second.error_id}: {second.reason}"
                await channel.publish(
                    CibaStateChangeEvent(
                        request_id=request_id,
...
def _render_result(agent_label: str, tool_id: str, result: ResultPayload) -> str:
    ...
    if tool_id == "hr.cubicle_summary":
        ...
        # Onboarding: IT kit fetched by hr_agent via its A2A call to it_agent.
        it_kit = data.get("it_kit")
        if it_kit:
            kit_lines = "; ".join(
                f"{k.get('model', '?')} ({k.get('catalogue_id', '?')}, {k.get('type', '?')})"
                for k in it_kit
            )
            reply += f"\n\nStandard new-hire IT kit (via IT Agent): {kit_lines}."
        return reply
